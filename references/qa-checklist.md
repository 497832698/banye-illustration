...
- **有 IP（固定般叶）**：双参考已传入（设定图 + 校准图）；锚点逐项可见 — **读 `ip/banye/ip.md`「锚点」+「QA·走形失败信号」**；服装配色以 IP 文件为准；比例与设定图一致
- 留白与色彩：**读 `styles/quirky-sketch.md`「QA 检查项」** 逐条执行

## 风格专属 QA

> 加载 `styles/quirky-sketch.md` →「QA 检查项」+「迭代 palette」（颜色失控时）

## IP 专属失败信号（般叶 · `$IP=banye`）

> 加载 `ip/banye/ip.md`；般叶走形项见该文件锚点与负向约束。

- **般叶锚点丢失**（银发变黄金灰、耳环数量/位置错、缺项链、头身比非 Q 版 1:1.3、戴了眼镜、表情单一）→ 改 prompt；读 `ip/banye/ip.md` 后重试
- **场景点睛色渗入角色服装**（背景蓝染白发、橙点染西装）→ 读 `styles/quirky-sketch.md`「迭代 palette」后重试
...
- **风格 QA 未过**（色纪律、标签、背景等）→ 读 `styles/quirky-sketch.md` QA + 迭代 palette 后重试
- **IP QA 未过**（锚点缺失、走形）→ 读 `ip/banye/ip.md` 后重试
- 标签超过 5 个 / 比例不对
...
- **颜色 / 线条 / 质感失控** → **读 `styles/quirky-sketch.md`「迭代 palette」**（不写死全局 prompt）
- **般叶配色走形** → **读 `ip/banye/ip.md`「QA · 走形失败信号」**
...
`v1.0` · 2026-07-30 · 自 gimi-illustration qa-checklist v1.9 派生；固定 IP=般叶，去除 gimi/none 专属信号。
