...
**双色层** — 般叶角色跟参考图全色；场景物件线稿为主 + 少量点色（见「颜色」）

## 颜色

| 层 | 规则 |
|----|------|
| **般叶角色** | 锚点色跟 **设定图 `reference-character.png`**（读 `ip/banye/ip.md`）；线条可 sketchy，**颜色不减、不被场景色染色** |
...
> 般叶专属锁色（不对称银环、银发、白西装、银项链、Q版比例）→ 读 `ip/banye/ip.md`「填入 `{IP_STYLE_ADAPT}`」，组装在 `{STYLE_ADAPT}` 之后。

## 般叶校准样板镜（已入库 · 仅重校准用）

> 般叶的怪诞手绘校准图（`assets/ip/banye/examples/quirky-sketch/banye-sample.png`）已生成并入库（QA 5/5 通过），
> 常规配图无需再跑本段。本段仅作**重校准 / 重新生成样板**时参考。

**必须载入：** 本节 `{STYLE_DNA}` + `{STYLE_ADAPT}` + 般叶 `reference-character.png`（全身）。

**画面规格（写死）：**

- 比例 16:9；般叶全身可见（可坐姿/侧身，不必站姿）
- 主题文案（固定）：vibe coding · 般叶配图风格样板
...
- [ ] 身份像 `reference-character.png`（锚点可辨）
...
- 填色物件 > 4；单物件填色 > 30%
- 橙 > 2 处，或橙用于物件填底 / 标签底
- **场景点睛色渗入般叶服装**（读 `ip/banye/ip.md` 判断具体走形项）
...
`v1.0` · 2026-07-30 · 自 gimi-illustration quirky-sketch v1.9 派生；$IP 固定 banye，去除 Gimi Tier 与自定义 IP 录入引用；般叶校准样板已入库。
